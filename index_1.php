...
function ajustarGrid() {
    ...
    // Medir el contenedor real (respeta max-width del .container)
    const areaAncho = grid.clientWidth - 86;
    const areaAlto = window.innerHeight - 74 - 12 - 80;
    const tilPorAncho = Math.floor((areaAncho - (cols - 1) * gap) / cols);
    const tilPorAlto = Math.floor((areaAlto - (filas - 1) * gap) / filas);
    const tileSize = Math.max(60, Math.min(tilPorAncho, tilPorAlto));
...
document.querySelector('.searchbar__input')?.addEventListener('input', function() {
    const q = this.value.toLowerCase().trim();
    document.querySelectorAll('.tile').forEach(tile => {
        const nombre = (tile.dataset.tileNombre || '').toLowerCase();
        tile.style.display = (!q || nombre.includes(q)) ? '' : 'none';
    });
});

document.querySelectorAll('[data-tile-menu-toggle]').forEach(btn => {
    btn.addEventListener('click', function(e) {
        e.preventDefault(); e.stopPropagation();
        const actions = this.closest('.tile__actions');
        actions.classList.toggle('is-open');
    });
});
document.addEventListener('click', () => {
    document.querySelectorAll('.tile__actions.is-open').forEach(a => a.classList.remove('is-open'));
});
</script>
</body>
</html>